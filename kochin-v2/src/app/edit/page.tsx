...
type Record = {
    id: number; name: string; absent: boolean;
...
export default function EditPage() {
  ...
  return (
    <main style={{ padding: '20px' }}>
      <h1 style={{ fontSize: '24px', marginBottom: '16px' }}>入力修正</h1>

      {/* 日付選択 */}
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '20px' }}>
        <button onClick={() => changeDate(-1)} style={{ padding: '8px 16px', fontSize: '18px', cursor: 'pointer' }}>◀</button>
        <input type="date" value={date} onChange={e => setDate(e.target.value)}
          style={{ padding: '8px', fontSize: '18px', borderRadius: '8px', border: '1px solid #ccc' }} />
        <button onClick={() => changeDate(1)} style={{ padding: '8px 16px', fontSize: '18px', cursor: 'pointer' }}>▶</button>
      </div>
...
<td style={td}>
  ...
  <td style={td}>
    <button
      onClick={() => updateRecord(r.id, 'am_absent', !r.am_absent)}
      style={{
...
{tab === 'pm' && (
  <td style={td}>
    <button
      onClick={() => updateRecord(r.id, 'pm_absent', !r.pm_absent)}
      style={{
        padding: '6px 12px', fontSize: '14px', borderRadius: '6px', border: 'none', cursor: 'pointer',
...
{tab === 'am' && !r.am_absent && <>
                ...
                {tab === 'pm' && !r.pm_absent && <>
                  ...
                  <td style={td}>
                    <select value={Number(r.pm_hours)} onChange={e => updateRecord(r.id, 'pm_hours', Number(e.target.value))}
                      style={{ fontSize: '14px', padding: '4px' }}>
                      {HOURS.map(h => <option key={h} value={h}>{h}h</option>)}
                    </select>
                  </td>
